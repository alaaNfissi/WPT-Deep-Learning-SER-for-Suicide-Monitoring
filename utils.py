...
def load_data(data_path):
    ...
    if data['source'].unique()[0] == 'IEMOCAP':
        data['label'] = data['label'].replace('exc', 'hap')
        data = data[data['label'].isin(['ang', 'hap', 'neu', 'sad'])].reset_index()
    
    elif data['source'].unique()[0] == 'CRISE':
        vad_paths = []
        vad_labels = []
        vad_sources = []
        for i in tqdm(range(len(data))):
            path = os.path.abspath(f"CRISE_VAD/{data['label'][i]}")
            try:
                os.makedirs(path)
            except FileExistsError:
                pass
            path_aux = os.path.join(path, data['path'][i].split('/')[-1].split('.')[0]+'_vad.wav')
            label = data['label'][i]
            source = data['source'][i]
            sm, vad_paths, vad_labels, vad_sources = write_audio(data['path'][i], path_aux, vad_paths, vad_labels, vad_sources, label, source)

        vad_df = pd.DataFrame({'path' : vad_paths, 'source' : vad_sources, 'label' : vad_labels})
        vad_df.reset_index(inplace=True)
        data = vad_df
