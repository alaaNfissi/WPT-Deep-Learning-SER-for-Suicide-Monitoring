...
"""
Title: Main Execution Script for Speech Emotion Recognition
Author: Alaa Nfissi
Date: July 21, 2024
Description: The main script to train, validate, and test the speech emotion recognition model. 
...
def train_WPTDL(config, checkpoint_dir=None, data=None, max_num_epochs=None):
    
    """
    Trains the model with given configuration, data, and number of epochs.
    ...
    model_WPTDL = WPTDL(n_input=config['n_input'], hidden_dim=config['hidden_dim'], n_layers=config['n_layers'] , n_output=config['n_output'], inputSize=None, kernelInit=config['kernelInit'], kernTrainable=config['kernTrainable'], level=config['level'], kernelsConstraint=config['mode'], initHT=config['initHT'], trainHT=config['trainHT'], alpha=config['alpha'])
    ...
    if torch.cuda.is_available():
        ...
        if torch.cuda.device_count() > 1:
            model_WPTDL = nn.DataParallel(model_WPTDL) # Use DataParallel for multi-GPU
    model_WPTDL.to(device) # Move model to the chosen device
    
    # Initialize optimizer and learning rate scheduler
    optimiser = optim.Adam(model_WPTDL.parameters(), lr=0.00001, weight_decay=0.00001)
    ...
    if checkpoint_dir:
        ...
        model_WPTDL.load_state_dict(model_state)
    ...
    with tqdm(total=config["num_splits"]) as pbar_1:
        for fold, (train_loader, val_loader) in enumerate(dataloaders, 0):
            ...
            with tqdm(total=epoch_count) as pbar:
                ...
                for epoch in range(1, epoch_count + 1):
                    ...
                    model_WPTDL.train() # Set model to training mode
                    right = 0 # Track number of correct predictions
    
                    h = model_WPTDL.init_hidden(config["batch_size"]) # Initialize hidden states
                    ...
                    for batch_index, (data, target) in enumerate(train_loader):
                        ...
                        output, h = model_WPTDL(data, h) # Forward pass
        ...
        ###################################################################################################
                    ...
                    # Validation loop
                    model_WPTDL.eval() # Set model to evaluation mode
                    right = 0 # Reset correct predictions count
                    val_loss = 0.0 # Reset validation loss
                    val_steps = 0 # Reset validation steps count
                    h = model_WPTDL.init_hidden(config["batch_size"]) # Reinitialize hidden states
                    ...
                    with tune.checkpoint_dir(epoch) as checkpoint_dir:
                        ...
                        torch.save((model_WPTDL.state_dict(), optimiser.state_dict()), path) # Save model and optimizer states
...
def main(num_samples=10, max_num_epochs=10, gpus_per_trial=1):
    ...
    # Start Ray Tune run
    result = tune.run(
        tune.with_parameters(train_WPTDL, data=data, max_num_epochs=max_num_epochs),
        resources_per_trial={"cpu": 12, "gpu": gpus_per_trial},
        config=config,
        num_samples=num_samples,
        scheduler=scheduler,
        progress_reporter=reporter,
        local_dir=os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_WPTDL"),
        log_to_file=(os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_WPTDL_stdout.log"), os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_WPTDL_stderr.log")),
        name="IEMOCAP_WPTDL",
        resume='AUTO')
    ...
    WPTDL_test_acc_result, y_pred, y_true = test(best_trained_model, best_trial.config["batch_size"], data)
    
    # Print test set accuracy
    print("Best trial test set accuracy: {}".format(WPTDL_test_acc_result))
